...
def check_for_non_numbers(split_input):
    ...
    for num in split_input:
        try:
            ...
            print(f'Error: "{num}" is not a number. Please try again.')
            Error = True
            break
    return Error
...
def time_until_str(target_datetime):
    ...
    if seconds >= 86400:
        return "in a day"
    if seconds >= 7200:
        return f"in {round(seconds/360)} hours"
    if seconds >= 3600:
        return "in an hour"
    if seconds >= 120:
    ...
    if seconds <= -86400:
        return "in a day"
    if seconds <= -7200:
        return f"{abs(round(seconds/360))} hours ago"
    if seconds <= 3600:
        return "in an hour"
    if seconds <= -120:
...
if __name__ == "__main__":
    ...
    print("---------------------RESULT----------------------------------------------")
    print("Input to Discord:              Example Result (what discord will display):")
    print(
